-- Add `research` (OpenAlex papers from Bay institutions) and `fda` (510(k)
-- device clearances from Bay companies) to the origin vocabulary.
--
...
-- both.
CREATE TABLE IF NOT EXISTS _mig11_comments AS SELECT * FROM comments;
CREATE TABLE IF NOT EXISTS _mig11_votes    AS SELECT * FROM story_votes;
CREATE TABLE IF NOT EXISTS _mig11_sources  AS SELECT * FROM story_sources;
CREATE TABLE IF NOT EXISTS _mig11_stories  AS SELECT * FROM stories;
CREATE TABLE IF NOT EXISTS _mig11_cvotes   AS SELECT * FROM comment_votes;
...
CREATE TABLE stories (
  ...
  origin TEXT NOT NULL DEFAULT 'bay'
         CHECK (origin IN ('bay','hn','lobsters','rss','event','github','sec','reddit','research','fda')),
...
CREATE TABLE story_sources (
  ...
  origin TEXT NOT NULL CHECK (origin IN ('bay','hn','lobsters','rss','event','github','sec','reddit','research','fda')),
...
SELECT id,kind,title,url,url_hash,body,author_id,origin,event_id,summary,topics_json,slug,
  image_url,description,site_name,favicon_url,author_name,published_at,lang,preview_fetched_at,
  vote_count,comment_count,score,dead,created_at FROM _mig11_stories;
INSERT INTO story_sources SELECT * FROM _mig11_sources;
INSERT INTO story_votes   SELECT * FROM _mig11_votes;
INSERT INTO comments      SELECT * FROM _mig11_comments;
INSERT INTO comment_votes SELECT * FROM _mig11_cvotes;
...
DROP TABLE _mig11_stories; DROP TABLE _mig11_sources; DROP TABLE _mig11_votes;
DROP TABLE _mig11_comments; DROP TABLE _mig11_cvotes;
